...
class SGEmbedding(nn.Module):
    """
    multi-graph spatial embedding        #####多图空间嵌入
    SE:     [num_vertices, D]
...
class STAttBlock(nn.Module):
    ...
    def forward(self, X, SGE):     #X是五个图的权重
...
class FusionGraphModel(nn.Module):
    ...
    def forward(self):
        ...
        SE = torch.from_numpy(np.float32(pd.read_csv('./data/SE/SE.csv', header=None).values)).to('cpu')
